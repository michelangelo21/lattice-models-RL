...
class XYmodelEnv(gym.Env):
    ...
    def __init__(self):
        ...
        self.observation_space = spaces.Box(
            0, np.pi, (SIDE_LENGTH, SIDE_LENGTH), dtype=np.float32
        )
        ...
        self.action_space = spaces.Box(
            -np.pi / 2, np.pi / 2, (SIDE_LENGTH * SIDE_LENGTH,), dtype=np.float32
        )
    ...
    def compute_energy(self, lattice):
        ...
        for i in range(SIDE_LENGTH):
            for j in range(SIDE_LENGTH):
                ...
                energy -= np.cos(lattice[i, j] - lattice[i - 1, j]) + np.cos(
                    lattice[i, i] - lattice[i, j - 1]
                )
        return energy
